...
public class FragmentationMagician {
    ...
    private static void hookStateSaved(FragmentManager fragmentManager, Runnable runnable) {
        ...
        if (isStateSaved(fragmentManager)) {
            boolean tempStateSaved = fragmentManagerImpl.isStateSaved();
            boolean tempStopped = fragmentManagerImpl.isStateSaved();

            try {
                //            fragmentManagerImpl.mStateSaved = false;
                //            fragmentManagerImpl.mStopped = false;
                Field mStateSavedfield  = fragmentManagerImpl.getClass().getDeclaredField("mStateSaved");
                Field mStoppedfield  = fragmentManagerImpl.getClass().getDeclaredField("mStateSaved");
                mStateSavedfield.setAccessible(true);
                mStoppedfield.setAccessible(true);
                mStateSavedfield.set(fragmentManagerImpl,false);
                mStoppedfield.set(fragmentManagerImpl,false);

                runnable.run();

//                fragmentManagerImpl.mStopped = tempStopped;
//                fragmentManagerImpl.mStateSaved = tempStateSaved;
                mStoppedfield.set(fragmentManagerImpl,tempStopped);
                mStateSavedfield.set(fragmentManagerImpl,tempStateSaved);
